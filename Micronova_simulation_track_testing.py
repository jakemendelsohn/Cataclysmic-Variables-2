# -*- coding: utf-8 -*-
"""
Created on Wed Mar  5 10:20:35 2025
...
def LC_model(name):
    ...
    return peak_lumi, integral_low,integral_up,outburst_duration,duration_lower,time,flux,time_array,flux_array
...
def simulation(iterations,name):
    ...
    # Prompt the user, but we don't need to store the input if we only
    # care about pressing Enter. Using input() will block until something is entered.
        input(f"Press Enter to increment the counter (currently {current_count})...")
    
        # After the user presses Enter, increment the counter and continue.
        peak_lumi,integral_low, integral_up,duration_upper,duration_lower,time,flux,time_array,flux_array = LC_model(name)
        ...
        plt.scatter(0.5 * (int_up_array + int_low_array),lum_array)
        plt.scatter(0.5* (integral_up+integral_low),peak_lumi,color = "red")
        central_x = 1.2*10**(39)
        central_y = 3.5*10**(34)
